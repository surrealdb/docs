...
Full-Text search is a feature that allows search capabilities within a database. This feature can be customized with specific analyzers that use tokenizers and filters to refine the search process.
...
In order to perform Full-Text Search on textual data, SurrealDB leverages analyzers, tokenizers, fiters and indexes to optimize search precision. It also use different search functions to highlight keywords and improve their relevance.

## Analyzers

[Analyzers](/docs/surrealdb/surrealql/statements/define/analyzer) are configurations that use tokenizers and/or filters to prepare text data for searching.

An analyzer is foundational to how text is processed in Full-Text Search. It is defined by its name, a set of tokenizers, and a set of filters.
...
If we tokenize the sentence '*Getting started with SurrealDB*' based on blank spaces, it would be broken down into the tokens Getting, started, with, and SurrealDB.
...
[Filters](/docs/surrealdb/surrealql/statements/define/analyzer#filters) process tokens for further refinement, such as converting to lowercase, removing special characters or breaking down words into their root forms to prepare for more effective searching.
...
To make a text field searchable, you need to set up a [full-text index](/docs/surrealdb/surrealql/statements/define/indexes/) on it by using the 'search' keyword.
